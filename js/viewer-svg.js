...
var TetrisViewer = {
	...
	initialise: function ( tetrisModel, elementId) {
	    var snap, board, row, column, elementRef, element, blockWidth, blockHeight;
        ...
        element.append('<svg style="width: ' + element.width() + 'px; height: ' + element.height() + 'px;"/>');

        snap = Snap(elementRef + ' svg');

		board = tetrisModel.getBoard();

        blockWidth = snap.node.clientWidth / board[0].length;
        blockHeight = snap.node.clientHeight / board.length;
		...
		for (row=0; row<board.length; row++) {
			for (column=0; column<board[0].length; column++) {
			    snap.rect(
			        blockWidth * column,
			        blockHeight * row,
			        (blockWidth * column) + blockWidth,
			        (blockWidth * row) + blockHeight).attr({
    			        id: 'r' + row + 'c' + column,
    			        class: 'block'
			        });
			}
